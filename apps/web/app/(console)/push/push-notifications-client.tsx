...
export function PushNotificationsClient() {
  ...
  const [provider, setProvider] = React.useState('fcm');
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      const [devicesRes, logsRes] = await Promise.all([
        fetch(`/api/push/devices?platform=${platform}&provider=${provider}&limit=50`, {
          cache: 'no-store',
        }),
        fetch(`/api/push/logs?platform=${platform}&provider=${provider}&limit=50`, {
          cache: 'no-store',
        }),
      ]);
