  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 group ${
          isOpen
            ...
            : 'bg-gradient-to-br from-violet-600 to-indigo-700 text-white hover:scale-110 hover:shadow-violet-500/30'
        }`}
      >
        <span
        ...
        {!isOpen && (
          <span className="absolute inset-0 rounded-full bg-violet-500 animate-ping opacity-20 group-hover:opacity-0" />
        )}
      ...
      >
        ...
        >
          {/* ── Header ──────────────────────────────────────────────────── */}
          <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-violet-600 to-indigo-700 text-white">
          ...
          {!isMinimized && (
            <>
                  ...
                  <div className="flex flex-col gap-1.5 mt-2">
                    ...
                    {SUGGESTED_QUESTIONS.map((q) => (
                      <button
                        ...
                        className="text-left text-xs px-3 py-2 rounded-xl border border-border/60 bg-muted/40 hover:bg-muted/80 hover:border-violet-400/50 text-foreground transition-all duration-150 hover:shadow-sm"
                      >
                        {q}
                      </button>
                    ))}
              ...
              {/* ── Input area ──────────────────────────────────────────── */}
              <div className="p-3">
                <div className="flex items-end gap-2 rounded-xl border border-border/60 bg-muted/30 px-3 py-2 focus-within:border-violet-400/60 focus-within:bg-background transition-all duration-200">
                  <textarea
                    ref={inputRef}
                    id="chat-input"
                    value={inputValue}
                  ...
                  <button
                    ...
                    className="flex-shrink-0 w-8 h-8 rounded-lg bg-gradient-to-br from-violet-600 to-indigo-700 text-white flex items-center justify-center hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-150 hover:scale-105 active:scale-95"
                  >
                    {isLoading ? (
