...
export class PerformanceAnalyzer {
  calculatePerformanceMetrics(
    ...
    return {
      ...
      sortinoRatio,
      calmarRatio: annualizedReturn.div(maxDrawdown.eq(0) ? new Decimal(0.001) : maxDrawdown),
  ...
  calculateTradeAnalysis(trades: Order[]): TradeAnalysis {
    ...
    return {
      ...
      largestWin: wins.length > 0 ? wins.reduce((max, win) => win.gt(max) ? win : max) : new Decimal(0),
      largestLoss: losses.length > 0 ? losses.reduce((max, loss) => loss.gt(max) ? loss : max) : new Decimal(0),
  ...
  private calculateTotalReturn(snapshots: PortfolioSnapshot[]): Decimal {
    ...
    return lastSnapshot.totalValue.sub(firstSnapshot.totalValue)
  ...
  private calculateAnnualizedReturn(snapshots: PortfolioSnapshot[]): Decimal {
    ...
    const daysDiff = (lastSnapshot.timestamp.getTime() - firstSnapshot.timestamp.getTime()) 
      / (1000 * 60 * 60 * 24);
    ...
    return totalReturn.pow(1 / years).sub(1).mul(100);
  }
    ...
    for (const snapshot of snapshots) {
      if (snapshot.totalValue.gt(peak)) {
        ...
        if (inDrawdown) {
          inDrawdown = false;
          if (drawdownStart) {
            const duration = (snapshot.timestamp.getTime() - drawdownStart.getTime()) 
              / (1000 * 60 * 60 * 24);
