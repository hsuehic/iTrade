...
export class SpreadGridStrategy extends BaseStrategy<SpreadGridParameters> {
  ...
  private getPendingLongRemaining(): Decimal {
    ...
    // 2. In-flight orders (generated by strategy, but exchange hasn't confirmed creation yet)
    for (const clientId of this.pendingClientOrderIds) {
      if (this.orders.has(clientId)) continue; // Already counted above
      ...
      if (
        ...
        // We assume all SpreadGrid entries have size this.orderAmount
        total = total.add(new Decimal(this.orderAmount));
      }
    }
