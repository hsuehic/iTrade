...
export class EventBus extends EventEmitter {
  ...
  public emitStrategyError(strategyName: string, error: Error): boolean {
    return this.emit(EVENTS.STRATEGY_ERROR, { strategyName, error, timestamp: new Date() });
  }
  ...
  public emitEngineStarted(): boolean {
    return this.emit(EVENTS.ENGINE_STARTED, { message: 'Trading engine started', timestamp: new Date() });
  }
  
  public emitEngineStopped(): boolean {
    return this.emit(EVENTS.ENGINE_STOPPED, { message: 'Trading engine stopped', timestamp: new Date() });
  }
  
  public emitEngineError(error: Error): boolean {
    return this.emit(EVENTS.ENGINE_ERROR, { message: 'Trading engine error', error, timestamp: new Date() });
  }
  ...
  public onEmergencyStop(callback: (data: { reason: string; timestamp: Date }) => void): this {
