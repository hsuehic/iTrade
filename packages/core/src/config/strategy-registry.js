...
export const STRATEGY_REGISTRY = {
    moving_average: {
        ...
        documentation: {
            overview: 'Uses two moving averages to identify trend changes. Generates buy signals when fast MA crosses above slow MA, and sell signals when fast MA crosses below slow MA.',
            parameters: 'FastPeriod should be smaller than slowPeriod. Common combinations: 5/20, 10/30, 12/26.',
            signals: 'BUY: Fast MA > Slow MA with sufficient threshold. SELL: Fast MA < Slow MA.',
            riskFactors: [
    ...
    rsi: {
        ...
        documentation: {
            overview: 'RSI oscillator identifies overbought and oversold conditions. Generates contrarian signals.',
            parameters: 'Standard period is 14. Overbought level typically 70-80, oversold level 20-30.',
    ...
    macd: {
        ...
        documentation: {
            overview: 'MACD combines trend and momentum analysis. Uses MACD line crossovers with signal line.',
            parameters: 'Standard settings are 12/26/9. FastPeriod < slowPeriod.',
            signals: 'BUY: MACD crosses above signal line. SELL: MACD crosses below signal line.',
            riskFactors: [
    ...
    bollinger_bands: {
        ...
        documentation: {
            overview: 'Uses volatility bands around a moving average to identify overbought/oversold conditions.',
    ...
    custom: {
        ...
        documentation: {
            overview: 'Highly customizable strategy with support for all parameter types. Demonstrates date pickers, color selectors, range sliders, enums, and more.',
            parameters: 'Configure strategy using visual controls like sliders, date pickers, and color selectors. Advanced users can define custom logic in JSON format.',
 ...
 */
export function getImplementedStrategies() {
...
/** 获取策略的默认参数 */
export function getStrategyDefaultParameters(type) {
...
export function getStrategiesByCategory(category) {
    return Object.values(STRATEGY_REGISTRY).filter((config) => config.category === category);
}
/**
 * 🎯 类型导出
 *
