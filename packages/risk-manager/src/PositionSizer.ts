...
export class PositionSizer {
  ...
  calculateKellySize(params: KellyParams): Decimal {
    ...
    const kellyValue = winProbability.mul(payoffRatio).sub(lossProbability).div(payoffRatio);
  ...
  calculateOptimalF(
    ...
    const largestLoss = tradeOutcomes.reduce((min, outcome) => 
      outcome.lt(min) ? outcome : min, new Decimal(0)
    ).abs();
  ...
  private calculateGeometricMean(outcomes: Decimal[], f: Decimal, largestLoss: Decimal): Decimal {
  ...
  calculateCorrelationAdjustedSize(
    ...
    const maxCorrelatedValue = correlatedExposure.mul(maxTotalCorrelatedExposure.div(100));
    
    if (correlatedExposure.gte(maxCorrelatedValue)) {
      // Reduce position size proportionally
      const reductionFactor = maxCorrelatedValue.div(correlatedExposure.add(basePositionSize));
