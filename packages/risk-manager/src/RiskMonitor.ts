...
export class RiskMonitor extends EventEmitter {
  ...
  private async checkConcentrationRisk(snapshot: RiskSnapshot): Promise<void> {
    if (snapshot.concentrationRisk.gt(50)) { // 50% concentration threshold
      await this.emitAlert({
  ...
  private async checkLiquidityRisk(snapshot: RiskSnapshot): Promise<void> {
    if (snapshot.liquidityRisk.gt(20)) { // 20% liquidity risk threshold
      await this.emitAlert({
  ...
  private async emitAlert(alert: RiskAlert): Promise<void> {
    ...
    if (cooldown && now.getTime() - cooldown.getTime() < 300000) { // 5 minute cooldown
      return;
    }
  ...
  getDrawdownTrend(periods: number = 100): { dates: Date[], values: Decimal[] } {
    const recentSnapshots = this.snapshots.slice(-periods);
  ...
  generateRiskReport(): {
    ...
    return {
      ...
      alerts: Array.from(alertCounts.entries()).map(([type, count]) => ({ type, count })),
      recommendations
    };
  }
